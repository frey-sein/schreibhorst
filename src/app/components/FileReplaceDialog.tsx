...
export default function FileReplaceDialog({ itemId, itemName, onClose, onSuccess }: FileReplaceDialogProps) {
  ...
  const { replaceFile } = useFileStore();
  
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = e.target.files;
    if (files && files.length > 0) {
      // Prüfe, ob die Dateiendung übereinstimmt
      const originalExtension = itemName.split('.').pop()?.toLowerCase() || '';
      const newExtension = files[0].name.split('.').pop()?.toLowerCase() || '';
      
      if (originalExtension !== newExtension) {
        setError(`Die Dateiendung muss ${originalExtension} sein. Die hochgeladene Datei hat die Endung ${newExtension}.`);
        return;
      }
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
      // Prüfe, ob die Dateiendung übereinstimmt
      const originalExtension = itemName.split('.').pop()?.toLowerCase() || '';
      const newExtension = e.dataTransfer.files[0].name.split('.').pop()?.toLowerCase() || '';
  ...
  const handleReplace = async () => {
    ...
    try {
      ...
      // Datei ersetzen
      await replaceFile(itemId, selectedFile);
      ...
      setError('Fehler beim Ersetzen der Datei. Bitte versuchen Sie es erneut.');
      setIsUploading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/30 flex justify-center items-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-6 max-w-md w-full mx-4">
        <h3 className="text-lg font-medium mb-4">Datei ersetzen</h3>
        <p className="text-sm text-gray-500 mb-4">
          Sie ersetzen die Datei "{itemName}". Bitte wählen Sie eine Datei mit der gleichen Endung.
        </p>
