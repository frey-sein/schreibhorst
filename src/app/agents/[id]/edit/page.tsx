...
export default function EditAgentPage({ params }: PageProps) {
  ...
  useEffect(() => {
    // Lade den Agenten aus dem localStorage
    const loadAgent = () => {
      try {
        const agents = JSON.parse(localStorage.getItem('agents') || '[]');
        const agent = agents.find((a: Agent) => a.id === resolvedParams.id);
        if (agent) {
          // Stelle sicher, dass schedule initialisiert ist
          ...
          setFormData(initializedAgent);
        } else {
          // Fallback auf Mock-Daten, wenn kein Agent gefunden wurde
        ...
        console.error('Fehler beim Laden des Agenten:', error);
      }
      setIsLoading(false);
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const agents = JSON.parse(localStorage.getItem('agents') || '[]');
      // Finde und aktualisiere den bearbeiteten Agenten
      const updatedAgents = agents.map((a: Agent) => 
        a.id === resolvedParams.id ? { ...formData, id: resolvedParams.id } : a
      );
    ...
    return (
      <>
        ...
        <main className="flex h-screen pt-[72px]">
          <div className="w-full flex flex-col h-full bg-[#f0f0f0]">
            <div className="flex-1 flex items-center justify-center">
              <div className="text-gray-500">Agent nicht gefunden</div>
            </div>
          </div>
        </main>
      </>
          ...
          <div className="sticky top-[64px] z-20 h-[120px] p-6 border-b border-gray-100 bg-white/80 backdrop-blur-md">
            <div className="flex justify-between items-start gap-4 w-full">
              <div className="flex-1">
                <h2 className="text-xl lg:text-2xl font-light text-gray-900 tracking-tight">Agent bearbeiten</h2>
                <p className="text-xs lg:text-sm text-gray-500 mt-1 break-normal">
                  Bearbeiten Sie die Einstellungen für {agent.name}
                </p>
              </div>
            </div>
          </div>
